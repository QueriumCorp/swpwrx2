...
RUN mkdir -p /usr/local/src/swpwrx2

# Set the created directory as a Docker volume
VOLUME ["/usr/local/src/swpwrx2"]
...
RUN echo "pip install -r /usr/local/src/swpwrx2/requirements.txt" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "pip install -e /usr/local/src/swpwrx2" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "cd /usr/local/src/swpwrx2 && make compile_translations && cd /usr/local/src/xblock-sdk" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
